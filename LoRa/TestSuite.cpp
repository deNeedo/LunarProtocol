...
void powerConsumption() {
  if (millis() > 30000 && powerOn) powerOn = false;
  if (DEVICE_ID == 0) diagnoseDevice(0, 1);
  else if (DEVICE_ID == 1 && powerOn) diagnoseDevice(0, 1);
  else radio.sleep();
}
...
bool diagnoseDevice(uint8_t SID, uint8_t RID) {
  ...
  if (DEVICE_ID == RID) {
    if (receivePacket(&pktIn, 1)) {
      ...
      if (powerOn) {sendPacket(&pktOut); return true;}
    }
  }
  return false;
}
